...
SELECT `car_year_id`, `car_year_name`, `car_year_created_at`, `car_year_updated_at`, `car_year_deleted_at`, `car_year_admin_status` FROM `cars_years` WHERE 1    
    */
    
    
    $xcrud->columns('car_year_id,car_year_name,car_year_admin_status');
    $xcrud->fields('car_year_id,car_year_name,car_year_admin_status');
    
    $search = @$_REQUEST['search'];

    $xcrud->where("car_year_id like '$search%' or car_year_name like '$search%'");
    
    $xcrud->order_by('car_year_id','desc');

    $xcrud->label('car_year_id','Year Id');
