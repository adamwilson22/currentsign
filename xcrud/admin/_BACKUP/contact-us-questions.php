...
<?php 
    ...
    $xcrud->table('contact_us_questions');
    
    // $xcrud->columns('id,email,first_name,country_code,mobile,dob,wallet');
    
    // $xcrud->fields('first_name,is_admin,wallet,status_admin');

//SELECT `cuq_id`, `cuq_question`, `cuq_created_at`, `cuq_updated_at`, `cuq_deleted_at` FROM `contact_us_questions` WHERE 1

    $search = @$_REQUEST['search'];
    
    $xcrud->where("cuq_question like '$search%'");


     $xcrud->order_by('cuq_id','desc');
