...
<?php 
    ...
    $xcrud->table('about_privacy');
    
    $xcrud->columns('name,description');
    $xcrud->fields('name,description');

    //SELECT `id`, `name`, `description`, `name_sp`, `description_sp` FROM `about_privacy` WHERE 1

    $search = @$_REQUEST['search'];
    $xcrud->where("name like '$search%' 
