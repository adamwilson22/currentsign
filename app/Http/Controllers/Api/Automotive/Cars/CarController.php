<?php
namespace App\Http\Controllers\Api\Automotive\Cars;
...
{
    public function get_car_classes(Request $request)
    {
        if (!Auth::guard('api')->check()) {
            return $this->sendError($result = [], $message = 'Unauthorized.', $notification = [], $error = [], $respose_code = 200);
        }
        $categories = DB::table('cars_classes')
            ->whereNull('car_class_deleted_at')
            ->where('car_class_admin_status', 'ACTIVE')
            ->get();
        if ($categories) {
